...
describe('LayerCreationCoordinator', () => {
	...
	it('builds only final canonical intent and never leaks the draft identity', () => {
		...
		assert.equal(coordinator.beginCommit(), true)
		assert.equal(coordinator.beginCommit(), false)
		assert.equal(coordinator.getSnapshot().commitPending, true)
		coordinator.endCommit()
		assert.equal(coordinator.getSnapshot().commitPending, false)
		assert.equal(coordinator.beginCommit(), true)
		assert.equal(coordinator.cancel(), true)
		assert.equal(coordinator.getSnapshot().commitPending, false)
	})
