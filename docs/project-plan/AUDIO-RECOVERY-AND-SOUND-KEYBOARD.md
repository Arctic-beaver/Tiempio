...
This plan records the two defects reported from the packaged Windows application on 2026-08-11:
...
The UI target is the retained light prototype reference
...
- Octave and key rotation live in a separate quiet control row immediately above the strip, so the
  keys remain the visual subject.
- Pitch, physical-key and tonic/held semantics remain truthful and accessible.
- Song Palette and the project Play panel continue using the standard/full performance treatment.
...
### Stage B — prototype-aligned Sound Chooser keyboard

Branch: `fix/sound-chooser-keyboard-strip`.

- Add a shared compact-strip presentation to `PerformanceKeyboard` without changing its input
  contracts.
- Extract/reuse the accessible rotation and octave controls so Sound Chooser can place them outside
  the key strip.
...
- Add a quiet Sound Chooser keyboard dock header with the play hint, rotation stepper and octave
  stepper.
- Keep standard Song Palette and full project keyboard layouts unchanged.
- Add source/policy or component-contract coverage for strip key count, control placement and the
  absence of filler keys.

### Stage C — integrated acceptance and package
...
- Keyboard tests and UI policy checks prove exactly seven compact strip mappings, no filler keys and
  no regression in standard/full layouts.
...
- Sound Chooser again presents a simple seven-key bottom strip with octave/rotation controls outside
  it; other performance surfaces do not visually regress.
